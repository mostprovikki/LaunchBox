...
| `cwd` from **in-row `cwd`**, first-wins | `:222-223` | **Critical:** the directory-name encoding maps `/`, `.` and `_` all to `-` and is **not invertible**. Folder-name decoding (`:293-295`) is a lossy last resort only — on this machine it would yield `/Users/vignesh/5036/mydevelopment/claude/scheduler`. Keep the warning comment. **[measured] The fallback is live, not theoretical: it fires on 3 of 59 real sessions** (224-byte stubs carrying no `cwd` on any row), and because the home directory name itself contains a hyphen the result is a path that **does not exist on disk**. So the parser returns a `cwdGuessed` flag: a resume that runs `cd <cwd>` would land nowhere, and a UI that renders it unqualified is lying about where the session lives. |

...
CREATE TABLE IF NOT EXISTS runs (
  ...
  meta TEXT,
  progress TEXT,
  -- What happened to the BEAD this run was launched for (lib/projects.js's
  -- onDone). NULL on every ordinary job run and on every row written before
  -- this column existed — "this run was not a bead run, or nobody recorded".
  -- A column rather than a meta key because it is the thing a Projects view
  -- filters and counts on, and because meta is the runner's namespace.
  -- See BEAD_OUTCOMES for the closed set.
  beadOutcome TEXT,
  createdAt TEXT NOT NULL
);
...
const RUN_COLS = [
  ...
  'meta', 'progress', 'beadOutcome',
];

// What the scheduler did to a bead after its run ended — the distinction the
// run's `status` cannot carry, because `ok` is true of both a bead that closed
// and one that came straight back to the backlog (WIP.md: a run whose every
// write was denied still exits `success`).
//
//   closed       bd close succeeded; the work is done and the bead is shut.
//   handed-back  the bead is open again and `bd ready` will offer it: either
//                the run failed, or it exited ok without TASK-COMPLETE.
//   stranded     the bead is stuck `in_progress`, which `bd ready` excludes,
//                so only a human can resolve it — a refused `bd close` after
//                real work, or an un-claim that would not go through.
//
// NULL is a fourth, legitimate state and is not in this list: not a bead run,
// or a row from before the column existed.
export const BEAD_OUTCOMES = Object.freeze(['closed', 'handed-back', 'stranded']);
...
function migrate(db) {
  ...
  // Same reason as readyCount below: `runs` already exists on every installed
  // db, so adding beadOutcome to SCHEMA alone would reach a fresh install only.
  // Left NULL on existing rows, which is exactly what they are — runs that
  // finished before anyone recorded what became of their bead.
  if (rcols.length && !rcols.includes('beadOutcome')) {
    db.exec('ALTER TABLE runs ADD COLUMN beadOutcome TEXT');
  }

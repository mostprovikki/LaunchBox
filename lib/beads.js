...
export function normaliseBead(b) {
  return {
    ...
    // The two long text fields. Carried rather than dropped because the review
    // queue reads `notes` — Task 4's skill writes an evidence note whose first
    // line is `run: gates passed|gates failed|no gates — branch <name>`, and
    // that line is what decides whether a finished branch may be offered for a
    // one-click merge. Dropped, every bead read as "no note", which the queue
    // could not tell apart from "gates failed". Null, never undefined, so a
    // consumer can distinguish "no note" from "never carried".
    notes: b.notes ?? null,
...
// "This bead does not exist" is an answer, not a failure. Both measured 1.1.0
// phrasings, because they differ between the two streams (see `get`).
const NOT_FOUND = /no issues? found|not found|no such issue/i;
  ...
  return {
    ...
    async get(project, beadId) {
      ...
      if (!res.ok) {
        ...
        //
        // Measured on 1.1.0, because the previous guess matched neither stream and
        // so never fired: stdout carries `{"error":"no issues found matching the
        // provided IDs"}` and stderr `Error fetching <id>: no issue found matching
        // "<id>"`. Both are checked — bd splits its error reporting across the two.
        if (NOT_FOUND.test(res.stderr) || NOT_FOUND.test(res.stdout)) return null;

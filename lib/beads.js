...
function invoke({ execFileFn, bdPath, args, beadsDir, cwd, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    const env = { ...process.env, BD_NON_INTERACTIVE: '1' };
...
function must(res, what) {
  if (!res.ok) {
    throw new BeadsError(`bd ${what} failed (exit ${res.exitCode}): ${res.stderr.trim() || 'no stderr'}`,
  ...
  return {
    ...
    async healthy(project) {
      try {
        ...
        return {
          ...
          databasePath: null,
          prefix: null,
          reason: busy ? 'beads database is busy (another bd or dolt process holds it)' : (err?.message ?? String(err)),
        };
      }
    },
    ...
    async claim(project, beadId) {
      ...
      if (!res.ok) {
        ...
        return {
          ...
          bead: null,
          reason: res.stderr.trim() || `exit ${res.exitCode}`,
        };
      }
    ...
    async close(project, beadId, { reason } = {}) {
      ...
      try {
        const res = await call(project, args);
        if (!res.ok) {
          return { ok: false, busy: false, reason: res.stderr.trim() || `exit ${res.exitCode}` };
        }

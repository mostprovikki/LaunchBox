...
export function createWorktrees({ execFileFn = execFile, timeoutMs = GIT_TIMEOUT_MS } = {}) {
  ...
  return {
    ...
    async snapshot(project, { root, beadId }) {
      ...
      // The commit above already landed on the bead branch — nothing is lost —
      // but a null sha here would let `committed: true` reach reap() with no way
      // to report the problem, so a reviewer relying on `meta.snapshotted` hits a
      // dead end. Throwing routes it through reap()'s existing catch, which
      // emits `snapshot-failed` with this reason.
      if (!sha.ok) throw new WorktreeError(`snapshot committed in ${path} but HEAD is unreadable: ${sha.stderr.trim()}`, { stderr: sha.stderr });
      return { path, committed: true, sha: sha.stdout.trim() };
    },

...
// Exported (additively — nothing about the values changes) so
// GET /api/v2/plan-candidates can state the fallback and the confidence
// threshold it is applying instead of a UI re-typing either number.
export const ASSUMED_COST_PCT = 1;
...
export function createBudgetPolicy({ db, usage = null, now = () => Date.now() }) {
  ...
  // The blocking reason for a scheduled fire of `job` given `snap`, as
  // structured data — `{code, ...values, message}` — or null when nothing
  // blocks. Split out from admit() so `explain()` can report exactly what a
  // fire would hit right now without pretending to run one.
  //
  // `message` (the English sentence) is composed HERE, from the very same
  // values carried alongside it, so the prose and the machine-readable form
  // cannot disagree. Consumers that need to render a reason read `code` and
  // the values; only a human-facing string reads `message`. That is the whole
  // point of claude-scheduler-ddu: before this, server.js regex-parsed these
  // sentences back apart to rebuild the values it is now simply handed, so a
  // reword here silently degraded every /api/v2 reason to `{code:'other'}`.
  //
  // `code` and the value keys ARE the contract now — renaming one is the
  // breaking change the wording used to be. The wording is free to change.
  function blockDetail(job, snap) {
    const s = settings();
    if (s.pauseOnWarning) {
      const hot = (snap.buckets ?? []).find((b) => b.isActive && ['warning', 'critical'].includes(b.severity));
      if (hot) {
        const bucket = bucketLabel(hot);
        return {
          code: 'bucket_severity',
          bucket, percent: hot.percent, severity: hot.severity,
          message: `paused: ${bucket} at ${hot.percent}% (${hot.severity})`,
        };
      }
    }
    for (const [key, reserveKey] of Object.entries(RESERVE_KEY)) {
      ...
      if (typeof pct === 'number' && pct >= num(reserveKey)) {
        const windowLabel = RESERVE_LABEL[key];
        // Deliberately the same keys the historical-row decoder produces for
        // this code, and no more: a live reason and a remembered one must be
        // the same shape, or a UI reading an extra field would render live
        // reasons and stored ones differently.
        return {
          code: 'reserve',
          windowLabel, usedPct: pct,
          message: `reserving ${windowLabel} headroom (${pct}% used)`,
        };
      }
    }
    ...
    if (Number.isFinite(min) && min > 0) {
      ...
      if (Number.isFinite(worst) && 100 - worst < min) {
        // Rounded once, here, and both the number and the sentence use that one
        // value — a second rounding downstream could disagree with the prose.
        const leftPct = Math.round((100 - worst) * 10) / 10;
        return {
          code: 'job_min_headroom',
          minHeadroomPct: min, leftPct,
          message: `job needs ${min}% headroom (${leftPct}% left)`,
        };
      }
    }
    return null;
  ...
  // The same decision as a bare sentence. Unchanged on purpose: /api/budget,
  // the scheduler's `meta.skipReason` recording and the current UI all consume
  // this exact string, so the structured form above is purely additive beside
  // it and never replaces it.
  function blockReason(job, snap) {
    return blockDetail(job, snap)?.message ?? null;
  }
  ...
  function explain(job = null) {
    ...
    // One evaluation, two shapes — so the sentence and the structured value can
    // never describe different decisions (e.g. a snapshot changing underneath
    // two separate calls).
    const detail = guard && !why ? blockDetail(job, snap) : null;
    return {
      ...settings(),
      enforcing: guard && !why,
      why: !guard ? 'guard is off' : why,
      blocked: detail?.message ?? null,
      // Additive twin of `blocked`: the same decision as `{code, ...values,
      // message}`. `blocked` stays the sentence every existing consumer reads;
      // callers that need to *render* the reason (server.js's /api/v2 surfaces)
      // read this instead of parsing that sentence back apart.
  ...
  // `blockDetail` is exported additively beside the rest: it is the structured
  // form of what `admit()`/`explain().blocked` say in English, for callers that
  // hold their own snapshot.
  return { admit, explain, blockDetail, plan, settings, snapshotProblem };
}

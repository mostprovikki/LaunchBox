...
export function createBudgetPolicy({ db, usage = null, now = () => Date.now() }) {
  ...
  function plan({
    ...
    jobIds = [], minGapMin = 10, maxConcurrent = 1,
  } = {}) {
    const ids = [...new Set((jobIds ?? []).filter(Boolean))];
    if (!ids.length) return { ok: false, reason: 'pick at least one job to spread the capacity across' };
    ...
    const avgCost = [...costs.values()].reduce((a, c) => a + c.cost, 0) / costs.size;
    ...
    for (let i = 0; i < timeSlots && slots.length < runs; i++) {
      ...
      for (let c = 0; c < concurrent && slots.length < runs; c++) {
        const jobId = ids[slots.length % ids.length];
        slots.push({ at, jobId, estPct: Number(costs.get(jobId).cost.toFixed(3)) });
      }
    }
  ...
  return { admit, explain, plan, settings };
}

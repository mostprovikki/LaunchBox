...
import {
  ...
  createJob, updateJob, findJobByBead, getSetting, getRun, updateRun,
} from './db.js';
...
// Stored when the declaration could not be read as an object at all. Those
// branches return `config: null` on purpose — `pollProject` reads that as
// "keep what we had", so a transiently unreadable file cannot knock out a
// working project — but `discover()` and POST /api/projects must store
// *something*, and `{}` was the wrong something: it re-validates to exactly
// one error, "must set autoLabel", so a trailing comma surfaced on the tab as
// a missing label. Holds messages, not the rejected values, so a repo cannot
// grant itself something (e.g. `ignoreGuard`) by writing this key itself.
export const REJECTED_KEY = '_rejected';
const REJECTED_MAX = 20;
const REJECTED_CHARS = 500;
const VALUE_CHARS = 40;
export const rejectedConfig = (errors) => ({ [REJECTED_KEY]: errors });
...
export function parseProjectConfig(raw) {
  ...
  if (permMode != null && !PERM_MODES.includes(permMode)) {
    const q = JSON.stringify(permMode) ?? String(permMode);
    const shown = q.length > VALUE_CHARS ? `${q.slice(0, VALUE_CHARS)}…` : q;
    errors.push(`${CONFIG_FILE}: defaults.permMode must be one of ${PERM_MODES.join('|')} (got ${shown})`);
  }
  ...
  // A previously-rejected declaration carries its reasons forward on the
  // *stored* config (`obj[REJECTED_KEY]`), because callers persist `config`,
  // not `errors` — without this, a rejected value silently normalised
  // (`permMode: "yolo"` → `"default"`) and every later re-parse of the stored
  // copy came back clean. Carried first so the order a caller first saw is
  // the order it keeps; de-duplicated by exact string so re-storing the same
  // rejection neither accumulates nor loses reasons; `[...all]` so the
  // returned verdict and the stored declaration cannot alias.
  const carried = Array.isArray(obj[REJECTED_KEY])
    ? obj[REJECTED_KEY]
      .filter((m) => typeof m === 'string' && m.trim())
      .slice(0, REJECTED_MAX)
      .map((m) => m.slice(0, REJECTED_CHARS))
    : [];
  const all = [...carried, ...errors.filter((e) => !carried.includes(e))];
  if (all.length) config[REJECTED_KEY] = [...all];
  return { ok: all.length === 0, errors: all, config };
}
...
export function beadPrompt(project, bead, { autoLabel, gates = null }) {
  return [
...
// How much a permission mode lets unattended work do, ascending. `auto` is
// `--dangerously-skip-permissions` (extensions/claude/index.js), so it tops the
// ladder.
const PERM_RANK = { default: 0, acceptEdits: 1, auto: 2 };
const permRank = (m) => PERM_RANK[m] ?? 0;

/**
 * Config refresh with `defaults.permMode` pinned for an ACTIVE project.
 *
 * The escalation this closes: every poll re-reads `.scheduler.json`, and an agent
 * the scheduler runs has write access in that repo. So it could add
 * `defaults.permMode: "auto"` to the very file that governs it, and its own
 * successors would then run with `--dangerously-skip-permissions` on the next
 * poll. The human approved *activation*, not "auto permissions from now on" — and
 * the invariant this module already states is that an agent may write
 * `.scheduler.json` while only a human may activate.
 *
 * Widening is refused and reported. Everything else in the file is adopted
 * freely, because a repo owning its own label, cwd and concurrency is the whole
 * point of the file. NARROWING is always allowed — a repo may make itself safer
 * without asking. A `pending` project has no approval to protect yet, so it
 * adopts whatever the file says; activation is what freezes the ceiling.
 */
export function pinPermMode(project, stored, fresh) {
  const asked = fresh?.defaults?.permMode;
  const kept = stored?.defaults?.permMode;
  if (project.state !== 'active') return { config: fresh, refused: false };
  if (!asked || permRank(asked) <= permRank(kept)) return { config: fresh, refused: false };
  return {
    config: { ...fresh, defaults: { ...(fresh.defaults ?? {}), permMode: kept } },
    refused: true,
    asked,
    kept: kept ?? 'default',
  };
...
export function createProjects({
  ...
  const PERM_REFUSAL = '.scheduler.json asks for permMode';
  function warnPermRefusal(projectId, { asked, kept }) {
    const w = `${PERM_REFUSAL} "${asked}" but this project was activated with "${kept}". Keeping "${kept}" — widening what unattended work may do to your files needs a human, so pause and re-activate to accept it.`;
    const had = warnings.get(projectId) ?? [];
    if (had.includes(w)) return;
    warnings.set(projectId, [...had, w]);
    events.emit('warning', { projectId, message: w });
  }

  // The declaration as the file says it NOW, unpinned, for activation to store
  // once the human has approved. Activation is the one moment a widened permMode
  // may be accepted: a paused project is not polled, so without this re-read the
  // copy stored at the first activation could never change — and the refusal
  // above tells the owner to pause and re-activate. `null` = unreadable: keep
  // what is stored, as the poller does for a transient read failure.
  async function readDeclaration(projectId) {
    const project = getProject(db, projectId);
    if (!project) return null;
    let raw;
    try {
      raw = await fsx.readFile(join(project.path, CONFIG_FILE), 'utf8');
    } catch {
      return null;
    }
    const parsed = parseProjectConfig(raw);
    return parsed.config ?? rejectedConfig(parsed.errors);
  }

  // Store what activation read, once the human has approved it. The approval
  // answers any earlier permMode refusal, so that warning goes with it — left up,
  // it told the owner to re-activate a project they just had.
  function adoptDeclaration(projectId, config) {
    const left = (warnings.get(projectId) ?? []).filter((w) => !w.startsWith(PERM_REFUSAL));
    if (left.length) warnings.set(projectId, left); else warnings.delete(projectId);
    return updateProject(db, projectId, { config });
  }
    ...
    for (const root of only ?? roots()) {
      ...
      for (const e of entries) {
        ...
        if (existing) {
          ...
          // activate something that is merely pending. And pin permMode exactly
          // as the poller does — this is the same re-read by another door.
          let config = parsed.config ?? rejectedConfig(parsed.errors);
          if (parsed.config) {
            const pinned = pinPermMode(existing, existing.config, parsed.config);
            config = pinned.config;
            if (pinned.refused) warnPermRefusal(existing.id, pinned);
          }
          const upd = updateProject(db, existing.id, { config, name: existing.name });
  ...
  async function pollProject(projectId, { max = null, trigger = 'beads' } = {}) {
    ...
    // `defaults.permMode` is PINNED for an active project — see pinPermMode.
    // Recorded here and reported after the warning reset below, which would
    // otherwise wipe it.
    let permRefusal = null;
    try {
      const raw = await fsx.readFile(join(project.path, CONFIG_FILE), 'utf8');
      const fresh = parseProjectConfig(raw);
      if (fresh.config) {
        const pinned = pinPermMode(project, stored, fresh.config);
        stored = pinned.config;
        if (pinned.refused) permRefusal = pinned;
        updateProject(db, project.id, { config: stored });
      }
    ...
    if (permRefusal) warnPermRefusal(project.id, permRefusal);
  ...
  function materialise(project, bead, config, cwd) {
    const params = {
      prompt: beadPrompt(project, bead, { autoLabel: config.autoLabel, gates: config.gates }),
  ...
  function onDone(project, bead, runId, { claimed = true } = {}) {
    // The outcome, on the run row. Every branch below already knew what became
    // of the bead and every one of them threw it away: the events reached a
    // console.log in server.js and nothing else, so no API field could tell a
    // closed bead from one that came back for retry (claude-scheduler-dc9).
    // Written BEFORE the matching event is emitted, so a listener that reads
    // the row sees the outcome rather than racing it.
    const outcome = (value) => updateRun(db, runId, { beadOutcome: value });
    runner.events.once(`done:${runId}`, async (status) => {
      try {
        if (status !== 'ok') {
          ...
          const returned = claimed ? await unclaim(project, bead.id, `run ${runId} finished ${status}`) : true;
          releaseLease(db, project.id, bead.id);
          // `handed-back` only if it really went back. A refused un-claim leaves
          // the bead `in_progress`, which `bd ready` excludes — recording that as
          // "returned to the backlog" would promise a retry that can never come.
          outcome(returned ? 'handed-back' : 'stranded');
        ...
        if (!signalledComplete(said, bead.id)) {
          ...
          const returned = claimed ? await unclaim(project, bead.id, `run ${runId} did not signal completion`) : true;
          releaseLease(db, project.id, bead.id);
          outcome(returned ? 'handed-back' : 'stranded');
        ...
        releaseLease(db, project.id, bead.id);
        // Best-effort: this catch exists because something already threw, and a
        // failing write here must not lose the release above.
        try { outcome('stranded'); } catch { /* the row is gone or unwritable */ }
        ...
        await reap(project, bead.id, runId);
      }
    });
  }
  ...
  return {
    events, start, stop, pollAll, pollProject, discover, readDeclaration, adoptDeclaration, refreshHealth, explain, recoverOrphans,

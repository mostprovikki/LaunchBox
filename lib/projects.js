...
export function createProjects({
  ...
  async function pollProject(projectId) {
    ...
    // There is ONE worktree per project, reused across runs, so two concurrent
    // beads from the same project would share a checkout and a branch and edit each
    // other's files — the exact collision the worktree exists to prevent, just
    // between two of our own runs instead of ours and the human's. Until worktrees
    // are per-bead, a project that isolates its work runs one bead at a time.
    const sharesAWorktree = !!(worktrees && setting('worktreeRoot', ''));
    const cap = sharesAWorktree ? 1 : config.maxConcurrent;
    if (sharesAWorktree && config.maxConcurrent > 1) {
      reasons.push(`running one bead at a time despite maxConcurrent: ${config.maxConcurrent} — this project's runs share a single git worktree, and two at once would edit the same files`);
    }
  ...
  async function tryRun(project, bead, config) {
    ...
    if (run.status === 'skipped') {
      ...
      const out = claimed ? await handBack(why) : abandon(why);

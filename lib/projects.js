...
export function createProjects({
    ...
    for (const root of only ?? roots()) {
      ...
      for (const e of entries) {
        ...
        if (existing) {
          ...
          const upd = updateProject(db, existing.id, { config: parsed.config ?? rejectedConfig(parsed.errors), name: existing.name });
  ...
  async function pollProject(projectId, { max = null, trigger = 'beads' } = {}) {
    ...
    if (permRefusal) {
      const w = `.scheduler.json asks for permMode "${permRefusal.asked}" but this project was activated with "${permRefusal.kept}". Keeping "${permRefusal.kept}" — widening what unattended work may do to your files needs a human, so pause and re-activate to accept it.`;
      warnings.set(project.id, [...(warnings.get(project.id) ?? []), w]);

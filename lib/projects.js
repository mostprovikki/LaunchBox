...
export const signalledComplete = (text, beadId) =>
  typeof text === 'string' && text.includes(completionMarker(beadId));
...
export function createProjects({
      ...
      try {
        ...
        if (!signalledComplete(said, bead.id)) {
          ...
          await beads.note(project, bead.id, [
            ...
            '',
            said ? said.trim() : '(the run produced no closing message)',
          ].join('\n'));
  ...
  async function recoverOrphans() {
    const out = [];
    for (const lease of listLeases(db, { state: 'held' })) {
      ...
      if (bead.assignee && bead.assignee !== BD_ACTOR) {
        out.push({ beadId: lease.beadId, handedBack: false, reason: `held by ${bead.assignee}, not us` });
        continue;
      }

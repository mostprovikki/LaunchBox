...
// `allowIgnoreGuard: false` is for a block a *repo* declared about itself, in its
// own committed `.scheduler.json`. The distinction is about whose resource is
// being spent. `permMode` is the repo saying what unattended work may do to *its
// own files*, under its own git review — fair for it to grant. The budget guard
// protects the account's headroom, which is shared across every project on this
// machine and is the human's to allocate. So a repo may make itself *more*
// conservative (`minHeadroomPct` only ever restricts) but may not exempt itself
// from the guard. And this is an error rather than a silent strip, for the same
// reason an unrecognised `permMode` is: a config that quietly means something
// weaker than it says is worse than one that refuses to load.
export function budgetParams(raw, errors, { label = 'params.budget', allowIgnoreGuard = true } = {}) {
  const b = raw?.budget;
  ...
  const out = {};
  if (b.ignoreGuard) {
    if (allowIgnoreGuard) out.ignoreGuard = true;
    else errors.push(`${label}.ignoreGuard cannot be declared by a project — the budget guard protects account-wide headroom shared with every other project, so exempting a job from it is a per-job setting here, not a repo's to grant itself`);
  }
  if (b.minHeadroomPct != null && b.minHeadroomPct !== '') {
    const n = Number(b.minHeadroomPct);
    if (!intIn(n, 1, 99)) errors.push(`${label}.minHeadroomPct must be 1-99`);
